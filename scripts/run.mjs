...
function deploy(params) {
  ...
  switch (type) {
    case 'profile': {
      let profile = process.env.AWS_PROFILE_NAME;
      ...
      if (profile) {
        console.log(
          chalk.gray(
            `[Deploy] - AWS_PROFILE_NAME is set. AWS_PROFILE: ${profile}`,
          ),
        );
        shell.env['AWS_PROFILE'] = profile;
      } else {
        console.log(
          chalk.gray(
            `[Deploy] - AWS_PROFILE_NAME is set. AWS_PROFILE: Default`,
          ),
        );
      }
      break;
    ...
    case 'cli': {
      ...
      const checkList = [AWS_SECRET_KEY, AWS_ACCESS_KEY].some((v) => !v);
      if (checkList) {
        console.log(
          chalk.red(
